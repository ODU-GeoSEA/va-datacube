...
# Function to retrieve geographic reference points from a GeoTIFF file.
...
# Function to calculate resolution from geo reference points and GeoTIFF information.
...
# Function to convert paths in document to absolute paths.
def absolutify_paths(doc):
    for band in doc['image']['bands'].values():
        band['path'] = os.path.abspath(band['path'])
    return doc

# Function to create a metadata document for indexing.
def make_metadata_doc(path, product_name, product_type, platform_code):
...
# Function to generate an S3 URL.
def get_s3_url(bucket_name, obj_key):
    return 's3://{bucket_name}/{obj_key}'.format(
        bucket_name=bucket_name, obj_key=obj_key)

# Function to add a dataset to the datacube index.
...
def worker(config, path, product_name, product_type, platform_code, unsafe, queue):
    ...
    while True:
        try:
            ...
            data = make_metadata_doc(path, product_name, product_type, platform_code)
            if data:
            ...
            queue.task_done()
# Function to iterate through datasets and initiate worker processes.
def iterate_datasets(path, product_name, product_type, platform_code, config, unsafe):
...
# Main function to parse command line arguments and start the indexing process.
...
def main(path, product_name, product_type, platform_code, config, unsafe):
    logging.basicConfig(format='%(asctime)s %(levelname)s %(message)s', level=logging.INFO)
    iterate_datasets(path, product_name, product_type, platform_code, config, unsafe)
